/*
* Proyecto Final Laboratorio CGIHC - Planta Nuclear
*/
...
bool Start() {
    ...
    // Create window
    window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Proyecto Final Laboratorio CGIHC - Planta Nuclear", NULL, NULL);
    if (window == NULL)
    {
...
bool Update() {
    ...
    // Materiales mate y plásticos con Phong shading
    {
        ...
        material_mate->Draw(*mLightsShader);
    ...
    // Dibujar materiales metálicos y translucidos con Fresnel shading (diferente refraccion de luz)
    {
        ...
        glBindTexture(GL_TEXTURE_2D, turbina->getFirstDiffuseTextureID());
        glBindTexture(GL_TEXTURE_2D, material_translucido->getFirstDiffuseTextureID());
        glBindTexture(GL_TEXTURE_2D, maqCombust->getFirstDiffuseTextureID());
        glBindTexture(GL_TEXTURE_2D, grua->getFirstDiffuseTextureID());
